...
public class FaceRectView extends View {
    ...
    private static void drawFaceRect(Canvas canvas, DrawInfo drawInfo, int faceRectThickness, Paint paint) {
        ...
        paint.setTextSize(rect.width() / 12);
        ////////////////
        int myliveness = drawInfo.getLiveness();
        String str;
        if (myliveness==1) {
            str = "REAL";
            CameraActivity.green();
        } else {
            str = "FAKE";
            CameraActivity.red();
        }
        myliveness = drawInfo.getLiveness();
        if(myliveness < 0) {
            str = "UNK";
            CameraActivity.red();
        }
        CameraActivity.status=str;
        if (CameraActivity.tv1 != null) {
            CameraActivity.tv1.setText(CameraActivity.status);
